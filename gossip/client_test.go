...
func startGossipAtAddr(nodeID roachpb.NodeID, addr net.Addr, stopper *stop.Stopper, t *testing.T, registry *metric.Registry) *Gossip {
	...
	g := New(context.TODO(), rpcContext, server, nil, stopper, registry, hlc.NewClock(hlc.UnixNano))
...
func startFakeServerGossips(t *testing.T) (*Gossip, *fakeGossipServer, *stop.Stopper) {
	...
	local := New(context.TODO(), lRPCContext, lserver, nil, stopper, metric.NewRegistry(), hlc.NewClock(hlc.UnixNano))
...
func TestClientGossip(t *testing.T) {
	...
	c := newClient(context.TODO(), remote.GetNodeAddr(), makeMetrics(clock), clock)
...
func TestClientGossipMetrics(t *testing.T) {
	...
	gossipSucceedsSoon(
		...
		map[*client]*Gossip{
			newClient(context.TODO(), local.GetNodeAddr(), remote.nodeMetrics, clock): remote,
		},
		func() error {
...
func TestClientNodeID(t *testing.T) {
	...
	rpcContext := rpc.NewContext(&base.Context{Insecure: true}, nil, stopper)
	clock := hlc.NewClock(hlc.UnixNano)
	c := newClient(context.TODO(), &remote.nodeAddr, makeMetrics(clock), clock)
	disconnected := make(chan *client, 1)
	disconnected <- c
...
func TestClientRegisterWithInitNodeID(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		gnode := New(context.TODO(), RPCContext, server, resolvers, stopper, metric.NewRegistry(), hlc.NewClock(hlc.UnixNano))
...
func TestClientForwardUnresolved(t *testing.T) {
	...
	clock := hlc.NewClock(hlc.UnixNano)
	client := newClient(context.TODO(), addr, makeMetrics(clock), clock) // never started
